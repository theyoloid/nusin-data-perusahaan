    ...
    <section class="bg-blue-50 ">
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
            <div class="grid text-center mb-6 gap-2">
                ...
                <h1 class="text-lg font-semibold leading-tight tracking-tight text-stone-600">
            ...
            <div class="mx-auto bg-white rounded-lg shadow md:mt-0 sm:max-w-4xl xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    <form class="space-y-4 md:space-y-6" method="post" action="{{ route('login') }}">
                        ...
                        {{-- Error Message --}}
                        @if ($errors->any())
                            <div class="bg-red-200 px-4 py-1 rounded-md">
                                <ul>
                                    @foreach ($errors->all() as $error)
                                        <li class="text-md text-red-900">{{ $error }}</li>
                                    @endforeach
                                </ul>
                            </div>
                        @endif
